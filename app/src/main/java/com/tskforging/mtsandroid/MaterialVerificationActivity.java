...
public final class MaterialVerificationActivity extends AppCompatActivity {
    ...
    private void draw(){body.removeAllViews();label("STEP 1 — ใบสั่งผลิต / ORDER SHEET",18,NAVY);button(order==null?"SCAN ORDER SHEET":"✓ ORDER: "+order.order,BLUE,v->scan(Target.ORDER));card(order==null?"ยังไม่สแกน":order.materialItem+"\n"+order.detail);
        label("STEP 2 — TAG เขียว / GREEN TAG",18,NAVY);button(green==null?"SCAN GREEN TAG":"✓ GREEN: "+green.materialItem,BLUE,v->scan(Target.GREEN));card(green==null?"ยังไม่สแกน":green.materialItem+"\n"+green.detail+"\nCharge: "+green.charge);
        label("STEP 3 — TAG เหลืองวัตถุดิบ / YELLOW TAG",18,NAVY);button(yellow==null?"SCAN YELLOW TAG":"✓ YELLOW: "+yellow.materialItem,BLUE,v->scan(Target.YELLOW));card(yellow==null?"ยังไม่สแกน":yellow.materialItem+"\n"+yellow.detail+"\nCharge: "+yellow.charge);
    ...
    private void scan(Target t){target=t;ScanOptions o=new ScanOptions();o.setPrompt("Scan "+t);o.setDesiredBarcodeFormats(ScanOptions.QR_CODE);o.setOrientationLocked(false);scanner.launch(o);}
    ...
    private void verify(){List<String> lines=new ArrayList<>();boolean ok=true;ok&=check(lines,"Order No.",order.order,green.order);ok&=check(lines,"Material Item",order.materialItem,green.materialItem);ok&=check(lines,"Material Detail",order.detail,green.detail);ok&=check(lines,"Part No.",order.partNo,green.partNo);ok&=check(lines,"Part Name",order.partName,green.partName);ok&=check(lines,"Green↔Yellow Material Item",green.materialItem,yellow.materialItem);ok&=check(lines,"Green↔Yellow Detail",green.detail,yellow.detail);ok&=check(lines,"Green↔Yellow Weight/Unit",green.weight+green.weightUnit,yellow.weight+yellow.weightUnit);ok&=check(lines,"Green↔Yellow Qty/Unit",green.qty+green.qtyUnit,yellow.qty+yellow.qtyUnit);ok&=check(lines,"Green↔Yellow Charge",green.charge,yellow.charge);
